...
from omero_search_engine.api.auth.OMERO_connector.utils import connect_omero, verify_session
import json
...
def create_token(datasource, omename, password, session_id):
    ...
    if len(data)>0:
        # JWT expire after 2 hrs by default unless it has been configured  otherwise using
        # set_JWT_expire_time
        jwt_exp=search_omero_app.config.get("JWT_EXPIRE_TIME")
        if not jwt_exp:
            jwt_exp=120
        exp = int(round((datetime.datetime.utcnow() + datetime.timedelta(minutes=jwt_exp)).timestamp()))
        token = jwt.encode({**
            {'omename': omename, 'exp': exp},**data},
            current_app.config['SECRET_KEY'], "HS256")
        return jsonify({"token": token})
    else:
        return jsonify({"message": "could not verify", "code": 401, "Authentication": "login required"})
...
def check_tocken(token):
    ...
    if token_data.get("session_id"):
        if not verify_session(token_data.get("data_source"), token_data.get("session_id")):
    ...
    is_expired=False
    is_valid = True
    return {token_data.get("data_source"):{"is_valid": is_valid, "is_expired": is_expired, "user_groups": user_groups, "is_admin": is_admin,"user_id":user_id,"data_source":token_data.get("data_source")}}
