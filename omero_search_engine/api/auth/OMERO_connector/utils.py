...
def connect_omero(datasource, omename, password, session_id):
    ...
    if not session_id:
        conn = BlitzGateway(username=omename, passwd=password, host=host, port=port, secure=True)
    else:
...
def get_user_groups(conn):
    ...
    for g in conn.getGroupsMemberOf():
        #if not g.isPrivate():
        groups[g.getId()]={"name": g.getName()}#, "is_Private": g.isPrivate(), "is_Public": g.isPublic(),"owner":g.getOwner()})#,"owner_1":g.isOwned})
